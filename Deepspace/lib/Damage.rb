...
class Damage
	...
	# 	_weapons: WeaponType[], types of weapons to be discarded
	...
	# Parameters:
	# 	_nWeapons: Integer, number of weapons to loose
	...
	# 	If a list of weapon types is not available, the number of weapons to be
	# 	deleted decreases by 1
	# Parameters:
	# 	w: Weapon, the weapon to be removed
	# Return:
	# 	Nil
	# WIP Hace falta hacer más comprobaciones sobre el arma??
	def discardWeapon(w)
		if @weapons.length != 0
			position = arrayContainsType(weapons, w.type)
			if position != -1
			...
			if @nWeapons > 0
				@nWeapons -= 1
			#else
			#	puts "WARNING! You attempted to have negative weapons at Damage.discardWeapon()"
			end
		end
	end
	...
	# Returns:
	# 	Bool:	true, if damage has no effect
